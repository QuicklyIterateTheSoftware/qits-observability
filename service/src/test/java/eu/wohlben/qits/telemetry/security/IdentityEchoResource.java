 ...
 *
 * <p>Served under {@code /observability/api/test-identity}: the suite inherits the shipped {@code
 * quarkus.rest.path=/observability/api}, so a test path is prefixed exactly like a real one.
 *
 * <p>Hidden from the OpenAPI document. {@code OpenApiSchemaExportTest} generates {@code
 * docs/openapi.yml} from a running {@code @QuarkusTest}, which indexes the test classpath too — so
 * without this a test fixture would be published in the committed API document as if it were a real
 * operation, and any client generated from that document would grow a method for an endpoint no
 * deployment serves.
 */
@Path("/test-identity")
