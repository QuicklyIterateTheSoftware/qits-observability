...
/**
 * {@code /observability} → {@code /observability/}, and nothing else.
 *
 * <p>Quinoa mounts the web client at {@code /observability/*}, which does not match the bare segment — so
 * before this route existed, typing {@code /observability} into a browser answered 404 while
 * {@code /observability/} served the client. Upstream behaviour, but not a defensible surface: the segment
...
public class WebUiRedirect {
  ...
  void init(@Observes Router router) {
    router
        ...
        .handler(
            rc -> {
              // Vert.x path routes are trailing-slash tolerant: route("/observability") matches /observability/ too,
              ...
              if (!"/observability".equals(rc.request().path())) {
                rc.next();
                return;
              }
              ...
              rc.response()
                  .setStatusCode(301)
                  .putHeader("Location", query == null ? "/observability/" : "/observability/?" + query)
                  .end();
            });
  }
}
